            ...
            <div class="relative">
                <div class="flex pb-2 space-x-4 overflow-x-auto snap-x scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
                    @forelse($units as $unit)
                        <div wire:key="unit-{{ $unit->id }}" wire:click="selectUnit({{ $unit->id }})" class="snap-start flex-none w-72 p-4 bg-white rounded-xl shadow-md cursor-pointer hover:shadow-lg hover:scale-105 transition-all duration-200 {{ $selectedUnit == $unit->id ? 'border-2 border-blue-500 bg-blue-50' : 'border border-gray-200' }}" role="button" aria-label="Select room {{ $unit->name }}">
                            <div class="flex items-center justify-between">
                                <h4 class="text-sm font-semibold text-gray-900 truncate">{{ $unit->name }}</h4>
                                <span class="w-3 h-3 rounded-full {{ $unit->status == 'available' ? 'bg-green-500' : 'bg-red-500' }}" aria-hidden="true"></span>
                            </div>
                            <p class="mt-2 text-xs text-gray-600 truncate">{{ $unit->unitType->name ?? 'N/A' }}</p>
                            <div class="flex items-center justify-between mt-2">
                                <span class="text-xs {{ $unit->status == 'available' ? 'text-green-600' : 'text-red-600' }} font-medium">{{ ucfirst($unit->status) }}</span>
                                <span class="text-xs text-gray-500">Capacity: {{ $unit->capacity ?? 'N/A' }} <i class="bi bi-people"></i></span>
                            </div>
                        </div>
                    @empty
