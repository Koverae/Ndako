<div>
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title">{{ __('Add Guest') }}</h5>
            ...
            <div class="container-fluid mt-3">
                ...
                <div class="mb-3 d-flex justify-content-between align-items-center flex-wrap gap-2">
                    ...
                    <button class="btn btn-primary"
                        ...
                        {{ __('Add Guest') }} <i class="fas fa-user-plus ms-1"></i>
                    </button>
                </div>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-12 col-md-6" wire:key="guest-{{ $guest->id }}">
                        <div class="card shadow-sm hover-shadow-sm border cursor-pointer" wire:click="selectGuest('{{ $guest->id }}')">
                            <div class="card-body">
                                <div class="d-flex justify-content-between align-items-center">
                                    <h5 class="mb-2 text-truncate">
                                        {{ $guest->name }}
                                        <i class="bi bi-pencil-square ms-2 text-muted small"
                                           onclick="Livewire.dispatch('openModal', {component: 'channelmanager::modal.add-guest-modal', arguments: {{ $guest->id }}})"></i>
                                    </h5>
                                    @if($guest->bookings()->isActive()->count() >= 1)
                                        <span class="badge bg-success">Active</span>
                                    @endif
                                </div>
                                <p class="mb-1 text-truncate"><i class="bi bi-envelope me-1"></i> {{ $guest->email }}</p>
                                <p class="mb-1 text-truncate"><i class="bi bi-phone me-1"></i> {{ $guest->phone }}</p>
                                <p class="mb-0 text-truncate"><i class="bi bi-geo me-1"></i> {{ $guest->address ?? '-' }}</p>
                            </div>
                        </div>
                    </div>
                    @empty
